...
def show_today_goals():
    ...
    table = Table(
        title="📊 Today's Workout", box=box.ROUNDED, show_lines=True, style="cyan"
    )
    ...
    if total_goal_reps > 0:
        ...
        console.print(
            f"[bold]Overall:[/bold] {total_done_reps}/{total_goal_reps} reps  [{bar}]  {pct * 100:.0f}%  • Completed {completed_ex_count}/{len(schedule)}"
        )


def show_menu():
    """Display the main menu."""
    table = Table(title=" -- HeraCLIs -- ", box=box.ROUNDED, style="cyan")
...
def add_workout():
    ...
    if not schedule:
        console.print(
            "[yellow]No workout scheduled for today. Use 'Randomize today's workout' first.[/yellow]"
        )
        return

    table = Table(
        title="Log Reps - Today's Exercises",
        box=box.ROUNDED,
        show_lines=True,
        style="cyan",
    )
    ...
    for item in schedule:
        ...
        if item.get("tags"):
            tags_str = ", ".join(
                [f"[{t['color']}]{t['name']}[/{t['color']}]" for t in item["tags"]]
            )
    ...
    try:
        ...
        if order_choice == "0":
            ...
            if not all_exercises:
                console.print(
                    "[red]No exercises available! Please add some exercises first.[/red]"
                )
                return
            ...
            for ex in all_exercises:
                ...
                if ex.get("tags"):
                    tags_str = ", ".join(
                        [
                            f"[{t['color']}]{t['name']}[/{t['color']}]"
                            for t in ex["tags"]
                        ]
                    )
                all_table.add_row(
                    str(ex["id"]), ex["name"], ex.get("description", ""), tags_str
                )
            ...
            goal = (
                db.get_goal_by_exercise_id(selected_exercise["id"])
                if selected_exercise
                else None
            )
            today_done = (
                db.get_todays_reps_for_exercise(selected_exercise["name"])
                if selected_exercise
                else 0
            )
            ...
            if remaining > 0:
                ...
                default_reps = (
                    30 if "plank" in selected_exercise["name"].lower() else 20
                )

            reps = IntPrompt.ask(
                f"How many {selected_exercise['name']}?", default=default_reps
            )

            if db.add_workout(selected_exercise["name"], int(reps)):
                console.print(
                    f"[green]✅ Logged {reps} {selected_exercise['name']}[/green]"
                )
            else:
...
def random_workout():
    ...
    try:
        ...
        if num_exercises < 1 or num_exercises > max_exercises:
            console.print(
                f"[red]Please choose between 1 and {max_exercises} exercises.[/red]"
            )
            return

    except ValueError:
        console.print(
            "[red]Invalid number. Please enter a number between 1 and {max_exercises}.[/red]"
        )
        return
    ...
    while True:
        ...
        selected_exercises = random.sample(
            [ex["name"] for ex in all_exercises], num_exercises
        )
            ...
            if exercise_details:
                ...
                if exercise_details.get("tags"):
                    ...
                    for tag in exercise_details["tags"]:
                        tag_display.append(
                            f"[{tag['color']}]{tag['name']}[/{tag['color']}]"
                        )
                    tags_str = ", ".join(tag_display)

                reps_display = (
                    f"{suggested_reps} sec"
                    if "plank" in exercise_name.lower()
                    else f"{suggested_reps} reps"
                )
        ...
        action = Prompt.ask(
            "[bold]Accept this selection?[/bold]", choices=["s", "r", "c"], default="s"
        )
        ...
        if action == "s":
            ...
            console.print(
                "[bold blue]You can now see your schedule on the main screen and log your progress.[/bold blue]"
            )
            break
        elif action == "r":
...
def show_stats():
    ...
    def fmt_change(section: dict) -> str:
        ...
        color = (
            "green"
            if section["diff"] > 0
            else ("yellow" if section["diff"] == 0 else "red")
        )
    ...
    if stats["top_exercises"]:
        ...
        for exercise in stats["top_exercises"]:
            table.add_row(
                exercise["name"], str(exercise["total_reps"]), str(exercise["sets"])
            )
    ...
    if monthly_data:
        ...
        monthly_chart = "\n".join(lines)
        console.print(
            Panel(monthly_chart, title="📅 Last 12 Months", border_style="blue")
        )
...
def manage_exercises():
    ...
    while True:
        ...
        choice = Prompt.ask(
            "Choose option", choices=["1", "2", "3", "4", "5", "q"], default="q"
        )
...
def edit_tag_color():
    ...
    try:
        ...
        console.print(
            f"Current color: [{selected_tag['color']}]●[/{selected_tag['color']}] {selected_tag['color']}"
        )

        new_color = Prompt.ask(
            "New color (hex code, e.g., #ff0000)", default=selected_tag["color"]
        )

        if db.update_tag_color(tag_id, new_color):
            console.print(
                f"[green]✅ Updated color for {selected_tag['name']} to {new_color}[/green]"
            )
        else:
...
def update_exercise_tags():
    ...
    console.print(table)
    try:
        exercise_id = IntPrompt.ask(
            "Select exercise ID to update tags (q to cancel)", default="q"
        )
        ...
        if db.update_exercise_tags(exercise_id, tag_names):
            console.print(
                f"[green]✅ Updated tags for {selected_exercise['name']}[/green]"
            )
            if tag_names:
...
def adjust_goals():
    ...
    for ex in exercises:
        ...
        rows.append(
            {"id": ex["id"], "name": ex["name"], "daily": daily, "weekly": weekly}
        )
    ...
    try:
        ...
        progress = (
            db.get_goal_progress(ex_id) if db.get_goal_by_exercise_id(ex_id) else None
        )
...
def add_exercise_to_workout():
    ...
    if not available_exercises:
        console.print(
            "[yellow]No exercises available to add! All exercises are already in today's workout.[/yellow]"
        )
        return

    # Display available exercises
    table = Table(
        title="Add Exercise to Today's Workout", box=box.ROUNDED, style="cyan"
    )
    ...
    try:
        exercise_id = IntPrompt.ask(
            "Select exercise ID to add (q to cancel)", default="q"
        )
        ...
        selected_exercise = next(
            (ex for ex in available_exercises if ex["id"] == exercise_id), None
        )
        if not selected_exercise:
            console.print("[red]Invalid exercise ID[/red]")
            return
        ...
        suggested_reps = IntPrompt.ask(
            f"Suggested reps for {selected_exercise['name']}?", default=default_reps
        )

        if db.add_exercise_to_schedule(exercise_id, suggested_reps):
            console.print(
                f"[green]✅ Added {selected_exercise['name']} to today's workout![/green]"
            )
...
def remove_exercise_from_workout():
    ...
    table = Table(
        title="Remove Exercise from Today's Workout", box=box.ROUNDED, style="cyan"
    )
    table.add_column("#", style="magenta", justify="center", width=3)
    table.add_column("Exercise", style="cyan", width=20)
    table.add_column("Goal Reps", style="green", width=12)
    ...
    for exercise in schedule:
        ...
        unit = "sec" if "plank" in exercise["exercise_name"].lower() else "reps"
        goal_display = (
            f"{exercise['suggested_reps']} {unit}"
            if exercise["suggested_reps"]
            else "N/A"
        )
    ...
    try:
        order_choice = IntPrompt.ask(
            "Select exercise # to remove (q to cancel)", default="q"
        )
        ...
        # Find the selected exercise
        selected_exercise = next(
            (ex for ex in schedule if ex["order_index"] == order_choice), None
        )
        if not selected_exercise:
        ...
        # Confirm removal
        if Confirm.ask(
            f"Remove {selected_exercise['exercise_name']} from today's workout?"
        ):
            if db.remove_exercise_from_schedule(selected_exercise["exercise_id"]):
                console.print(
                    f"[green]✅ Removed {selected_exercise['exercise_name']} from today's workout![/green]"
                )
            else:
...
def edit_logged_reps():
    ...
    table = Table(
        title="Edit Today's Logged Reps", box=box.ROUNDED, show_lines=True, style="cyan"
    )
    table.add_column("#", style="magenta", justify="center", width=3)
    ...
    try:
        ...
        console.print(
            f"Date: {datetime.fromisoformat(selected_entry['date']).strftime('%Y-%m-%d %H:%M:%S')}"
        )
        ...
        if action == "q":
            ...
            new_reps = IntPrompt.ask(
                "Enter new number of reps", default=selected_entry["reps"]
            )
            ...
            # Confirm the change
            if Confirm.ask(
                f"Change {selected_entry['exercise_name']} from {selected_entry['reps']} to {new_reps} reps?"
            ):
                if db.update_workout_reps(selected_entry["id"], new_reps):
                    console.print(
                        f"[green]✅ Updated {selected_entry['exercise_name']} to {new_reps} reps[/green]"
                    )
                else:
            ...
            # Confirm deletion
            if Confirm.ask(
                f"Delete {selected_entry['exercise_name']} entry with {selected_entry['reps']} reps?"
            ):
                if db.delete_workout_entry(selected_entry["id"]):
                    console.print(
                        f"[green]✅ Deleted {selected_entry['exercise_name']} entry[/green]"
                    )
                else:
    ...
    if stats:
        ...
        try:
            ...
            if len(parts) >= 2:
                ...
                console.print(
                    "[red]Invalid format. Use: --add 'exercise_name reps'[/red]"
                )
        except ValueError:
...
def settings_menu():
    ...
    while True:
        ...
        if choice == "1":
            ...
            raw = Prompt.ask(
                "Choose by number or enter a path (blank to cancel)", default=""
            )
            if not raw:
                continue
            selected_path = None
